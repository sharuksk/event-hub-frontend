...
const CateringCard = ({
  ...
  const isSelected = selectedCaterings.some((c) => c.id === catering.id);
      ...
      <div className="bg-background/85 text-foreground p-[20px] rounded-[11px] w-[300px] border">
        ...
        >
          ...
          <Box sx={{ textAlign: "left" }}>
            ...
            <Typography variant="body2">Price: {catering.rent}</Typography>
            <Typography variant="body2">
              Available on:{" "}
              {catering.availableDates
...
const CateringSelection = ({
  ...
  const caterings = mockData.serviceData.filter(
    (service) => service.type === "catering"
  );

  const availableCaterings = caterings.filter((catering) =>
    selectedDates.some((date) => catering.availableDates.includes(date))
  );
